...
## Issue #81: EventStore Bulk-Write Benchmarking

### Benchmark Harness Shape (2026-04-15 | Alec)

**Decision:** Add a dedicated `benchmarks/Chaos.Mongo.EventStore.Benchmarks` BenchmarkDotNet project that:
- references the real `Chaos.Mongo.EventStore` project
- starts a MongoDB 8 replica-set container via Testcontainers
- compares `AppendWithoutBulkWrite` vs `AppendWithBulkWrite`
- enables checkpoints so each append exercises the multi-collection EventStore path
- rebuilds isolated databases per BenchmarkDotNet iteration instead of measuring container startup

**Rationale:** Issue #81 asked for targeted benchmarks on the real EventStore append path, not an isolated microbenchmark. This shape keeps the benchmark close to production wiring, while still making runs reproducible and bounded enough for local validation.

**Notes:**
- BenchmarkDotNet 0.15.6 required synchronous iteration hooks in practice, so setup/cleanup block on async Mongo/Testcontainers work internally.
- A `benchmarks/Directory.Build.props` override keeps the benchmark project single-targeted (`net10.0`) so BenchmarkDotNet's autogenerated project restores cleanly.

### EventStore Bulk-Write Opt-In API (2026-04-15 | Alec)

**Decision:** Expose the EventStore optimization as `MongoEventStoreBuilder<TAggregate>.WithBulkWriteOptimization()`, backed by `MongoEventStoreOptions<TAggregate>.BulkWriteOptimizationEnabled`.

**Rationale:**
- Matches the existing `With*()` fluent builder style already used across Chaos.Mongo feature packages.
- Keeps rollout explicitly opt-in instead of auto-detecting and silently changing behavior.
- When enabled, `MongoEventStore.AppendEventsAsync()` validates server support up front and throws a clear `NotSupportedException` on MongoDB versions older than 8.0 instead of falling back automatically.

**Implementation Notes:**
- The optimized path uses client-level bulk write models against BSON documents so MongoDB 8 can send EventStore's built-in writes in one request while preserving transaction semantics.
- `onBeforeCommit` remains a separate transactional callback and still runs after the built-in EventStore writes succeed.

### Team Directive: BenchmarkDotNet Preference (2026-04-15 | Christian Flessa)

**Directive:** Benchmark performance work should use BenchmarkDotNet as the preferred basis for further optimization.

**Authority:** User request — captured for team memory.
